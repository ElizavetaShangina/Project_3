...
class Dropdown(disnake.ui.StringSelect):
    def __init__(self):
        options = [
            disnake.SelectOption(label="Уровень 1", description="Для новичков и неуверенных в себе личностей",
                                 emoji="🟩"),
            disnake.SelectOption(label="Уровень 2", description="Для продвинутых игроков", emoji="🟨"),
            disnake.SelectOption( label="Уровень 3", description="Для тех, у кого слишком много времени и нервов",
                                  emoji="🟥"),]
        super().__init__(
...
class USER(commands.Cog):
    ...
    async def sign(self, ctx, operation, login, password):
        ...
        if operation == 'in':
            ...
            cur.execute(f"""INSERT INTO Passed_Levels (Login, Level_1, Level_2, Level_3) VALUES 
                            ('{login}', False, False, False)""").fetchall()
            con.commit()
            con.close()
    ...
    async def password(self, ctx, attempt):
        if self.attempt_number == 0:
            ...
            if attempt == '9413':
                await ctx.send(
                    'Вы слышите заветный писк - сейф взломан! На вашем лице появляется глупая улыбка - вы счастливы. '
                    'Вдруг в сейфе окажется карта, ведущая к выходу... От этой мысли вы замираете на несколько секунд. '
                    'Неужели, вы завершили квест? Вы выиграли в этой игре, длившейся, кажется, бесконечно долго? \n '
                    'Вы тряхнули головой. Зачем гадать? Вы же можете просто открыть сейф... Ваша рука тянется к дверце '
                    'сейфа. Взявшись за ручку, вы медлите еще буквально мгновение и открываете дверцу. '
                ...
                await ctx.send(
                    'Ноль. НОЛЬ! Попытки кончились, а вы так и не отгадали пароль! Надежда еще теплится в вашем '
                    ...
                    'вернуться к двери, которая вела в этот коридор. \nЧерез какое-то время вы дошли до двери. '
                    'Подергав ручку, вы понимаете, что дверь заперта. Вы разражаетесь безумным смехом - вы заперты в '
                    'этом коридоре... заперты навечно. Ваша судьба очевидна, как скоро бы она не наступила, и на этом'
                    ' ваш квест завершен.')
            ...
            if direction_3.value == 'направо':
                ...
                if level == 1:
                    await ctx.send(
                        ...
                        "Вы вспоминаете, что в вашем рюкзаке лежат артефакты, которые вы зачем-то подобрали во "
                        "время экспедиции. А вдруг они и есть ключи к этим вратам? С надеждой "
                    ...
                    if self.stone:
                        await ctx.send(
                            "Вы вспоминаете, что в вашем рюкзаке лежит камень квадратной формы, который вы зачем-то "
                            "подобрали в подземелье. А вдруг он и есть ключ к этим вратам? С надеждой вы достаете его "
                            "из рюкзака и... О, чудо! Он подошел! Врата жутко заскрипели и начали медленно "
                            "открываться... Вы застыли в изумлении - из-за ворот бьёт ослепляющий белый свет. Неужели "
    ...
    async def maze(self, ctx):
        ...
        if self.death_reason:
            ...
            await ctx.send('Для того чтобы еще раз пройти лабиринт введите "/sign in Ваш логин Ваш пароль". \n'
                           'Если вы хотите пройти лабиринт под именем уже существующего пользователя, этого будет '
                           'достаточно. \n '
                ...
                if level == 1:
                    if self.first_time:
                        await ctx.send(
                            ...
                            'Вы открываете вторую дверь и видите лестницу. Поднявшись по ней и открыв железную дверь, '
                            'вы оказываетесь на улице. Не удержавшись, делаете несколько шагов вперед, вдыхая полной '
                            'грудью. ')
